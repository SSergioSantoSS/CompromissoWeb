    ...
    public class HomeController : Controller
    {
        ...
        ([FromServices] IEventoRepository eventoRepository)
        {
            ...
            try
            {
                ...
                var dataMin = new DateTime
                (ano, mes, 1);
                var dataMax = new DateTime

                (ano, mes, DateTime.DaysInMonth(ano, mes));
                ...
                model.ConsultaPrioridade = eventoRepository

                .GroupByPrioridade
                (dataMin, dataMax, usuario.IdUsuario);
                model.ConsultaAtivoInativo = eventoRepository
                .GroupByAtivo(dataMin, dataMax, usuario.IdUsuario);
        ...
        public IActionResult Index(EventoDashboardModel model,
        [FromServices] IEventoRepository eventoRepository)
        ...
        {//verificar se todos os campos passaram nas regras de validação
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    .GroupByPrioridade(dataMin, dataMax, usuario.IdUsuario);
