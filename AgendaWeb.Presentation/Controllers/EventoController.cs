...
namespace AgendaWeb.Presentation.Controllers
{
    ...
    public class EventoController : Controller
    {
        ...
        public IActionResult Cadastrar(EventoCadastroModel model,
        [FromServices] IEventoRepository eventoRepository)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    = $"O evento '{evento.Nome}', foi cadastrado com sucesso!";
        ...
        [HttpPost]
        public IActionResult Consultar(EventoConsultaModel model,
        [FromServices] IEventoRepository eventoRepository)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    model.Eventos = eventoRepository.GetAll(dataMin, dataMax, ObterUsuarioAutenticado().IdUsuario, int.Parse(model.Ativo) );
            ...
            [FromServices] IEventoRepository eventoRepository)
        {
            try
            {
                ...
                {
                    //Alterar a flag ATIO(1) para INATIVO(0)
                ...
                {
                    TempData["MensagemErro"] = $"Evento não foi encontrado.";
                }
        ...
        public IActionResult Editar(Guid idEvento,
            [FromServices] IEventoRepository eventoRepository)
        {
        ...
        //Método para receber a requisição do formulário da página de Relatórios
        [HttpPost]
        public IActionResult Relatorio(EventoRelatorioModel model,
            [FromServices] IEventoRepository eventoRepository)
        {
            if (ModelState.IsValid)
            {
