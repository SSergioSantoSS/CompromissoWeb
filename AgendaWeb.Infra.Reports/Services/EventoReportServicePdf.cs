...
namespace AgendaWeb.Infra.Reports.Services
{
    ...
    {
        public byte[] GerarRelatorio(Usuario usuario,

        List<Evento> eventos, DateTime dataMin, DateTime dataMax)

        {
            ...
            using (var document = new Document(pdf))
            {
                ...
                ($"Data de início: {dataMin.ToString("dd/MM/yyyy")}"));

                document.Add(new Paragraph

                ($"Data de fim: {dataMax.ToString("dd/MM/yyyy")}"));
                ...
                foreach (var item in eventos)
                {
                    ...
                    table.AddCell(item.Prioridade == 0 ? "ALTA" :
                    item.Prioridade == 1 ? "MÉDIA" : "BAIXA");
